...
create_channel(){
    ...
    lncl alice openchannel --node_key=$bob_pubkey --local_amt=11000000 --push_amt=10000000
    docker-compose -f $COMPOSE_FILE run btcctl generate 10
}

get_macaroon(){
    local lndpath="/tmp/thor/lnd/lnd_$1/data/chain/bitcoin/simnet/admin.macaroon"
    local macaroon=$(xxd -ps -c 200 $lndpath | tr -d '\n')
    echo $macaroon
}
